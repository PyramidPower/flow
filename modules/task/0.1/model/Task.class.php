...
//	var $_versionable;		// Versionable Aspect
	...
	// helper function: return all pages attached to a task given a task ID
	function & getPages($id) {
		return $this->getObjects("TaskObject",array("task_id"=>$id,"table_name"=>"page"));
	}
    
	// return the number of pages attached to a task
	function countTaskPages() {
		$i = 0;
		$pages = $this->getPages($this->id);
		
		// some pages may not be accessible to me so try to get page and only count those which are returned
		if ($pages) {
			foreach ($pages as $page) {
				$pg = $this->w->Page->getPage($this->w,$page->object_id);
