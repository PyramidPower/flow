...
function task_listener_POST_ACTION(&$w) {
	...
	if ($w->currentHandler() == "task" && $w->ctx("TaskComment") && $w->ctx("TaskEvent")) {
		...
		if ($notifyusers) {
			...
			$subject = "New Task Event - " . $task->title . ": " . $eventtitle;
    	    $message ="<br/>\n";
        	$message .= "Task Event - " . $eventtitle . "<p>";
	        $message .= $comm->comment . "<p>";
			$message .= Html::a(WEBROOT."/task/viewtask/".$comm->obj_id, "View Task");
			$message .= "<p>&nbsp;<p><b>Note</b>: Go to " . Html::a("/task/tasklist/?tab=2","Task > Task List > Notifications") . ", to edit the types of notifications you will receive.";
